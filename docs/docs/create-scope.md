...
| `...` | `T...` | Initial items to track |

### `Scope`

Callable: `scope(item) -> item` tracks an item; `scope()` cleans up all items.

Cleans items by type: Instance (`:Destroy`), RBXScriptConnection (`:Disconnect`), function (call it), thread (`task.cancel`), table (tries common cleanup methods).
